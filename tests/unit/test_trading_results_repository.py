...
class TestRepo:
    ...
    @pytest.mark.parametrize("date, expected_result", [(date(2024, 4, 25), FAKE_TRADING_RESULTS[0]), (date(2024, 4, 26), None)])
    @pytest.mark.asyncio
    async def test_fetch_one_trading_result_by_date(self, clean_trading_results, add_trading_results, date, expected_result):
    ...
    @pytest.mark.parametrize("oil_id, delivery_type_id, delivery_basis_id, expected_result",
                             [(None, None, None, [FAKE_TRADING_RESULTS[0]]), ("A100", None, None, [])])
    @pytest.mark.asyncio
    async def test_get_last_results(self, clean_trading_results, add_trading_results, oil_id, delivery_type_id, delivery_basis_id, expected_result):
        async with async_db_session() as session:
            repo = TradingResultsRepository(session=session)
            await clean_trading_results()
            await add_trading_results()
            result = await repo.get_last_results(oil_id=oil_id, delivery_type_id=delivery_type_id, delivery_basis_id=delivery_basis_id)
            if result:
                result = [result.to_pydantic_schema() for result in result]
            assert result == expected_result
            await session.close()

    @pytest.mark.parametrize("oil_id, delivery_type_id, delivery_basis_id, start_date, end_date, expected_result",
    ...
    async def test_get_trading_results_in_period(
            self, clean_trading_results, add_trading_results, oil_id, delivery_type_id, delivery_basis_id, start_date, end_date, expected_result
    ):
        async with async_db_session() as session:
            repo = TradingResultsRepository(session=session)
            await clean_trading_results()
            ...
            result = await repo.get_trading_results_in_period(
                oil_id=oil_id, delivery_type_id=delivery_type_id, delivery_basis_id=delivery_basis_id, start_date=start_date, end_date=end_date
            )
            if result:
                result = [result.to_pydantic_schema() for result in result]
            assert result == expected_result
